...
do_umap <- function(fcs,subtype_markers,sample_ids,cell_clustering,metadata,
                    clusterMergeFile='C:/Users/jayal/OneDrive - Johns Hopkins/WJH Lab JSB/CXCR4 CyTOF/T Cells/Analysis/Config',
...
output <- returnfcs(metaDataFile = paste0(workd,"/Config","/metadata_T.xlsx"),
                    panelDataFile = paste0(workd,"/Config","/panel_T.xlsx"),
...
clusterMergeFile = paste0(workd,"/Config","/merged_T.xlsx")
...
metadata = read_xlsx('C:/Users/jayal/OneDrive - Johns Hopkins/WJH Lab JSB/CXCR4 CyTOF/T Cells/Analysis/Config/metadata_T.xlsx')
...
dgplots<-makeDiagnosticPlots(expr01, 
                             ...
                             fileName = 'T_plot_diagnostics.pdf')
...
mdsggp <- ggplot(ggdf, aes(x = MDS1, y = MDS2, color = timepoint, shape = batch)) +
  ...
  coord_fixed()
pdf("T_plot_mds.pdf", width=4, height=5);mdsggp;dev.off()
...
dev.off()
pdf("T_plot_meanheatmap.pdf", width=12, height=5);meanht;dev.off()
...
write.csv(counts, file='T_counts.csv')
write.csv(props, file='T_props.csv')
...
pdf("T_plot_abundance_box.pdf", width=10, height=8);boxplot;dev.off()

#line plot for all timepoints

##Abundance plot - Day 1 vs Day 8 
ggdf_gvax<-ggdf[ggdf$timepoint %in% c("C1D1", "C1D8"),]
ggdf_gvax$timepoint<- factor(ggdf_gvax$timepoint, levels=c("C1D1","C1D8","C1D15"))
comps<- list(c("C1D1","C1D8"))
comps2<- list(c("C1D8","C1D15"))
ggdf_gvax<-ggdf[ggdf$patient_id %nin% c("P8"),] ##Excluded due to missing data point
...
lineplot_r <- ggplot(ggdf_gvax, aes(x=timepoint, y=proportion, group=patient_id))+
  ...
  theme(axis.text.x = element_text(size=8, angle = 45, hjust=1, vjust=1, color="black"),
        ...
        legend.key = element_rect(fill="white"))
dev.off()
pdf("T_abundance_C1D1 vs C1D8.pdf", width=11, height=16);lineplot_r;dev.off()
...
lineplot_r <- ggplot(ggdf_gvax, aes(x=timepoint, y=proportion, group=patient_id))+
  ...
  theme(axis.text.x = element_text(size=8, angle = 45, hjust=1, vjust=1, color="black"),
        ...
        legend.key = element_rect(fill="white"))
dev.off()
pdf("T_abundance_C1D8 vs C1D15.pdf", width=11, height=16);lineplot_r;dev.off()
...
lineplot_r <- ggplot(ggdf_gvax, aes(x=timepoint, y=proportion, group=patient_id))+
  ...
  theme(axis.text.x = element_text(size=8, angle = 45, hjust=1, vjust=1, color="black"),
        ...
        legend.key = element_rect(fill="white"))
dev.off()
pdf("T_abundance_C1D1 vs C1D15.pdf", width=11, height=16);lineplot_r;dev.off()
...
lineplot_r <- ggplot(ggdf_gvax, aes(x=timepoint, y=proportion, group=patient_id))+
  ...
  theme(axis.text.x = element_text(size=8, angle = 45, hjust=1, vjust=1, color="black"),
        ...
        legend.key = element_rect(fill="white"))
dev.off()
pdf("T_abundance_C1D1 vs C1D8 vs C1D15.pdf", width=11, height=16);lineplot_r;dev.off()
...
dev.off()
pdf('T_plot_umaps.pdf',width=10,height=10)
...
##Functional Plot - Day 1 vs Day 8 vs Day 15
ggdf_gvax<-ggdf[ggdf$timepoint %in% c("C1D1", "C1D8","C1D15"),]
...
ggdf3 <- gsub('X41BB', "CD137", ggdf2_gvax$variable)
#Replace old column with new colummn
ggdf2_gvax$variable <- ggdf3

pdf("T_functional_C1D1 vs C1D8 vs C1D15.pdf",width=14,height=20)
...
##Functional Plot - Day 1 vs Day 8
ggdf_gvax<-ggdf[ggdf$timepoint %in% c("C1D1", "C1D8"),]
ggdf2_gvax<-ggdf2[ggdf2$patient_id %nin% c("P8"),] ##Excluded due to missing data point
ggdf2_gvax$timepoint<- factor(ggdf2_gvax$timepoint, levels=c("C1D1","C1D8","C1D15"))
#Replace 41BB for CD137 in DF and Make New Column
...
pdf("T_functional_C1D1 vs C1D8.pdf",width=14,height=20)
for(i in 1:length(fmlistplot)){
  ggp <- ggplot(ggdf2_gvax[ggdf2_gvax$variable==fmlistplot[i],], aes(x=timepoint,y=value, group=patient_id))+
    ggtitle(fmlistplot[i])+
    facet_wrap(~cluster, scales='free',ncol=4)+
...
##Functional Plot - Day 8 vs Day 15 
ggdf_gvax<-ggdf[ggdf$timepoint %in% c("C1D8", "C1D15"),]
ggdf2_gvax<-ggdf2[ggdf2$patient_id %nin% c("P8"),] ##Excluded due to missing data point
ggdf2_gvax$timepoint<- factor(ggdf2_gvax$timepoint, levels=c("C1D1","C1D8","C1D15"))
...
pdf("T_functional_C1D8 vs C1D15.pdf",width=14,height=20)
for(i in 1:length(fmlistplot)){
  ggp <- ggplot(ggdf2_gvax[ggdf2_gvax$variable==fmlistplot[i],], aes(x=timepoint,y=value, group=patient_id))+
    ggtitle(fmlistplot[i])+
    facet_wrap(~cluster, scales='free',ncol=4)+
...
##Functional Plot - Day 1 vs Day 15
ggdf_gvax<-ggdf[ggdf$timepoint %in% c("C1D1", "C1D15"),]
ggdf2_gvax<-ggdf2[ggdf2$patient_id %nin% c("P8"),] ##Excluded due to missing data point
ggdf2_gvax$timepoint<- factor(ggdf2_gvax$timepoint, levels=c("C1D1","C1D8","C1D15"))
...
pdf("T_functional_C1D1 vs C1D15.pdf",width=14,height=20)
for(i in 1:length(fmlistplot)){
  ggp <- ggplot(ggdf2_gvax[ggdf2_gvax$variable==fmlistplot[i],], aes(x=timepoint,y=value, group=patient_id))+
    ggtitle(fmlistplot[i])+
    facet_wrap(~cluster, scales='free',ncol=4)+
...
############# comparing counts by timepoints

########## PRE vs C1D1
celltable = read.csv("C:/Users/45072/Desktop/WJHlab/RoussosTorres CyTOF/single_cell.csv")
d1 = data[data$timepoint == "PRE",]
cluster_name = as.character(unique(d1$cluster))
ids = unique(d1$sample_id)
dx = d1[,-1]
d2 = dx[,-c(2,39:44)]
PRE = "PRE"
C1D1 = "C1D1"
WK8 = "KW8"
df=data.frame(matrix(nrow = 19,ncol = 0)) 

sample_rep = c()
for (i in 1:length(ids)) {
  sub_d2 = d2[d2$sample_id==ids[i],]
  sub_d22 = sub_d2[,-1]
  num = c(which(dx$sample_id==ids[i]))
  sample_rep = c(sample_rep,rep(as.character(dx[num[1],44]),ncol(sub_d22)))
  df = cbind(df,sub_d22)
}
sample_rep
clusters = c()
for (i in 1:length(ids)) {
  c = d1[d1$sample_id == ids[i],]
  name = unique(c$timepoint)
  clusters = c(clusters,rep(name,ncol(sub_d22)))
}



d = DGEList(counts=df,group=sample_rep)
...
for (i in 1:1) {
  clusternames = c(clusternames,row.names(counts_noWK8))
}


for (i in 1:1) {
  for (j in (i+1):2) {
    group = paste(clusters[i],' and ',clusters[j])
    for (k in 1:36) {
...
write.csv(edgeR_table,paste0("edgeR.csv"))
write.csv(FDR_adjusted,paste0("FDR.csv"))
write.csv(cluster_name,paste0("clustername.csv"))
write.csv(comparison,paste0("comparison.csv"))


##########
g = counts
g_PRE = g[,!grepl("_2",names(g))]
g_PRE = g_PRE[,!grepl("_3",names(g_PRE))]
g_PRE = g_PRE[,-13]
sampleName = colnames(g_PRE)
groupz = c()

for (i in 1:ncol(g_PRE)) {
  num = c(which(data$sample_id==sampleName[i]))
  groupz = c(groupz,as.character(data[num[1],45]))
}

d = DGEList(counts=g_PRE,group=groupz)
d0 <- estimateCommonDisp(d, verbose=T)
d0 <- estimateTagwiseDisp(d0)
...
#################### for abundance (counts)

celltable = counts
#celltable = celltable[!celltable$condition=="TNBC",]
#celltable = celltable[-which(is.na(celltable$response)),]
...
for (i in 1:length(unique(celltable$cluster))) {
  clusterz = unique(celltable$cluster)
  celltable_oneclu = celltable[celltable$cluster == clusterz[i],]
  celltable_oneclu1 = celltable_oneclu[,-1]
  celltable_oneclu1 = celltable_oneclu1[!celltable_oneclu$timepoint == "WK8",]  ### dropping unused parameters
  ids = unique(celltable_oneclu1$sample_id)
  celltable_oneclu2 = celltable_oneclu1[,-c(1,2,39:44)]
  indicator = which(rowSums(celltable_oneclu2)==0)
  if (length(which(rowSums(celltable_oneclu2)==0))>0) {
    celltable_oneclu2 = celltable_oneclu2[-which(rowSums(celltable_oneclu2)==0),]
    celltable_oneclu1 = celltable_oneclu1[-indicator,]
  }

  
  df = t(celltable_oneclu2)
  
  PRE = "PRE"
  C1D1 = "C1D1"
  WK8 = "WK8"
  marker_names = c(colnames(celltable_oneclu2))
  sample_survival = c(celltable_oneclu1$survival)
  sample_response=c(celltable_oneclu1$response)
  sample_timepoint=c(celltable_oneclu1$timepoint)
  sample_patient = c(celltable_oneclu1$sample_id)
  
  d = DGEList(counts=df,group=sample_timepoint)  ###DGEList object and group setting
  ...
  for (x in 1:1) {
    for (y in (x+1):2) {
      et <- exactTest(d, pair=c(x,y))
      fdr = p.adjust(et$table$PValue,method = "BH")
      FDR_adjusted = c(FDR_adjusted,fdr)
      fourCluster = rbind(fourCluster,et$table)
      mergedEDGER = rbind(mergedEDGER,et$table)
    }
    FDR = c(FDR,FDR_adjusted)
  }
  for (a in 1:length(unique(celltable$cluster))) {
    clusternames = c(clusternames,rep(clusterz[a],nrow(df)))
    
    two = "PRE vs C1D1"  ### change the comparison names in the final table
    compare = c(compare,rep(two,nrow(df)))
    
    funcmarker = c(funcmarker,marker_names)
  }
...
finaltable = data.frame(Cluster=clusternames,
                        ...
                        DFDR=FDR)
write.csv(finaltable,paste0("edgeR_singleCell_PREvsC1D1_allSamples1.csv"))  ### change the name of the file



################################################# diff analysis (normal paired t test) for single cell

pvalue<-data.frame(cluster=NA,marker=NA,Var1=NA,Var2=NA,value=NA)
#ggdf2_1 = ggdf2[ggdf2$condition=="HR",]
ggdf2_1 = ggdf2[ggdf2$timepoint=="PRE",]
for(i in 1:length(fmlistplot)){
  marker=fmlistplot[i]
  ggdf2_marker<-ggdf2_1[ggdf2_1$variable==marker,]
  for(j in 1:19){
    ggdf2subset<-ggdf2_marker[ggdf2_marker$cluster==clusterz[j],]
    if(which(ggdf2subset$response=="NA")>0){
      ggdf2subset=ggdf2subset[-which(ggdf2subset$response=="NA"),]
    }
    res<-pairwise.t.test(ggdf2subset$value, ggdf2subset$survival, p.adjust.method = "none")
    pvalue<-rbind(pvalue,cbind(marker=rep(fmlistplot[i],nrow(melt(res$p.value))),
                               cluster=rep(clusterz[j],nrow(melt(res$p.value))),
                               melt(res$p.value))
    )
    pvalue<-pvalue[!is.na(pvalue$value),]
  }
}
pvalue$FDRadjP<-p.adjust(pvalue$value, method="BH")
pvalue$cluster=as.character(pvalue$cluster)
pvalue = pvalue %>%
  mutate(cluster =  factor(cluster, levels = clusterz)) %>%
  arrange(cluster)
rownames(pvalue)=NULL
write.csv(pvalue,paste0("edgeR_singleCell_YvsN_onePRE.csv"))
